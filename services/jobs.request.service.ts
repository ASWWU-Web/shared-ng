...
export class JobsRequestService extends RequestService {
  ...
  readJob(jobId: number): Observable<JobView> {
    const jobObservable = super.get(`${this.baseURL}/job/view/${jobId}`).pipe(
      map((data: {form: JobView}) => data.form)
    );
  ...
  postSubmission(submissionData: ApplicationPOST): Observable<any> {
    const submissionObservable = super.post(`${this.baseURL}/application/submit`, submissionData, null, 'urlencoded');
